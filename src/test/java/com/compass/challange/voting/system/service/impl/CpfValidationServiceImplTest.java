...
class CpfValidationServiceImplTest {
    ...
    void setUp() {
        ...
        cpfValidationService = new CpfValidationServiceImpl(cpfValidationClient);
    }

    @Test
    void valiate_shouldReturnCpfValidation_whenClientReturnsResult() {
        String cpf = "12345678901";
        CpfValidation expected = new CpfValidation();
        expected.setStatus("ABLE_TO_VOTE");

        when(cpfValidationClient.validate(cpf)).thenReturn(expected);

        CpfValidation result = cpfValidationService.valiate(cpf);
